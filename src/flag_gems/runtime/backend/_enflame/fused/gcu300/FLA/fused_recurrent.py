...
def fused_recurrent_gated_delta_rule_fwd_sp_for_qwen3_next_kernel(
    ...
    if USE_INITIAL_STATE:
        if IS_CONTINUOUS_BATCHING:
            if IS_SPEC_DECODING:
                i_t = tl.load(num_accepted_tokens + i_n).to(tl.int32) - 1
            else:
                i_t = 0
            p_h0 = (
                h0
...
def fused_recurrent_gated_delta_rule_fwd_kernel(
    ...
    num_accepted_tokens,
    scale,
    N: tl.int32,  # num of sequences
    T: tl.int32,  # num of tokens
    B: tl.constexpr,
    H: tl.constexpr,
    HV: tl.constexpr,
    K: tl.constexpr,
...
def fused_recurrent_gated_delta_rule_fwd(
    ...
    # GCU300 does not support int64 data types. Cast the index tensors to
    # int32 before launching (no-op when they are already int32).
