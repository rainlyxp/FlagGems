...
_GENERIC_MODULE_NAME = "flag_gems.ops.mkldnn_rnn_layer"


class _MkldnnRnnLayerRedirectFinder(importlib.abc.MetaPathFinder):
    """Resolve ``flag_gems.ops.mkldnn_rnn_layer`` imports to the XPU override.

    The one-shot patch below is enough for the initial import, but some test
    modules purge ``flag_gems.*`` from ``sys.modules`` and re-import
    ``flag_gems`` at collection time (e.g.
    ``tests/test_nested_view_from_buffer_copy.py``). This override module stays
    cached under the ``_kunlunxin.*`` name and therefore never re-runs, so the
    patch would be lost. A ``sys.meta_path`` finder survives that purge (it only
    filters out ``ScikitBuildRedirectingFinder``) and re-routes every later
    import of the generic submodule to this override.
    """

    def find_spec(self, fullname, path=None, target=None):
        if fullname != _GENERIC_MODULE_NAME:
            return None
        return importlib.machinery.ModuleSpec(
            fullname,
            _MkldnnRnnLayerRedirectLoader(),
            is_package=False,
        )


class _MkldnnRnnLayerRedirectLoader(importlib.abc.Loader):
    """Return a module exposing the XPU ``mkldnn_rnn_layer`` override."""

    def create_module(self, spec):
        module = types.ModuleType(spec.name)
        module.mkldnn_rnn_layer = mkldnn_rnn_layer
        module.__all__ = ["mkldnn_rnn_layer"]
        return module

    def exec_module(self, module):
        # The override function is bound in create_module; nothing to execute.
        pass
...
def _install_redirect_finder():
    ...
    for finder in sys.meta_path:
        if isinstance(finder, _MkldnnRnnLayerRedirectFinder):
            return
    sys.meta_path.insert(0, _MkldnnRnnLayerRedirectFinder())
...
def _redirect_generic_entry_points():
    ...
    _generic_submodule = sys.modules.get(_GENERIC_MODULE_NAME)
    if _generic_submodule is not None:
        setattr(_generic_submodule, "mkldnn_rnn_layer", mkldnn_rnn_layer)

    # (3) keep the redirect alive across flag_gems re-imports.
