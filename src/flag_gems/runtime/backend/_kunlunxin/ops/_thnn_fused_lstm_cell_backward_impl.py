...
def _lstm_cell_bwd_b1_kernel(
    ...
    i_gate = tl.load(workspace_ptr + h, mask=mask, other=0.0).to(tl.float32)
    f_gate = tl.load(workspace_ptr + H + h, mask=mask, other=0.0).to(tl.float32)
    g_gate = tl.load(workspace_ptr + 2 * H + h, mask=mask, other=0.0).to(tl.float32)
    o_gate = tl.load(workspace_ptr + 3 * H + h, mask=mask, other=0.0).to(tl.float32)
    ghy = tl.load(grad_hy_ptr + offs, mask=mask, other=0.0).to(tl.float32)
    gcy = tl.load(grad_cy_ptr + offs, mask=mask, other=0.0).to(tl.float32)
    cxv = tl.load(cx_ptr + offs, mask=mask, other=0.0).to(tl.float32)
    cyv = tl.load(cy_ptr + offs, mask=mask, other=0.0).to(tl.float32)
    ...
    ty = grad_gates_ptr.dtype.element_ty
    tl.store(grad_gates_ptr + h, grad_i.to(ty), mask=mask)
    tl.store(grad_gates_ptr + H + h, grad_f.to(ty), mask=mask)
    tl.store(grad_gates_ptr + 2 * H + h, grad_g.to(ty), mask=mask)
    tl.store(grad_gates_ptr + 3 * H + h, grad_o.to(ty), mask=mask)
    tl.store(grad_cx_ptr + offs, grad_cx.to(grad_cx_ptr.dtype.element_ty), mask=mask)

    # batch_size == 1: grad_biases == grad_input_gates row 0 (no reduction needed)
    tyb = grad_biases_ptr.dtype.element_ty
    tl.store(grad_biases_ptr + h, grad_i.to(tyb), mask=mask)
    tl.store(grad_biases_ptr + H + h, grad_f.to(tyb), mask=mask)
    tl.store(grad_biases_ptr + 2 * H + h, grad_g.to(tyb), mask=mask)
    tl.store(grad_biases_ptr + 3 * H + h, grad_o.to(tyb), mask=mask)
...
def _thnn_fused_lstm_cell_backward_impl(
    ...
    with torch_device_fn.device(cx.device) if use_guard else contextlib.nullcontext():
        # For batch_size == 1 the bias is just grad_input_gates row 0, so fuse the
        # per-element backward and the bias copy into a single launch (saves a launch).
        fuse_b1 = has_bias and batch_size == 1 and N < 256
        if N > 0:
            if fuse_b1:
                BLOCK = 512
                grid = (triton.cdiv(N, BLOCK),)
                _lstm_cell_bwd_b1_kernel[grid](
                    grad_hy,
                    grad_cy,
                    cx,
                    cy,
        ...
        if has_bias and not fuse_b1 and batch_size > 0:
